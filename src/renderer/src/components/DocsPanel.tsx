...
export function DocsPanel({ docsAPI, onAddDocSource, onRecrawlDocSource }: DocsPanelProps) {
  ...
  useEffect(() => {
    const cleanup = window.claude.onDocsSetupProgress?.((progress: { step: string; detail?: string; stepIndex: number; totalSteps: number }) => {
  ...
  const getStatusBadge = (source: DocSourceInfo) => {
    ...
    switch (source.status) {
      ...
      case 'crawling': {
        ...
        return <span className={`${base} bg-[rgba(129,66,199,0.15)] text-[#a070d0]`}><Loader2 size={12} className={spinClass} /> {label}</span>
      }
      case 'error':
        return <span className={`${base} bg-[rgba(255,80,80,0.15)] text-[#ff5050]`}>Error</span>
      case 'pending':
        return <span className={`${base} bg-[rgba(255,180,50,0.15)] text-[#ffb432]`}>Pending</span>
      default:
        return <span className={`${base} bg-[rgba(255,255,255,0.05)] text-[#888]`}>{source.status}</span>
    }
  }

  const sectionTitleClass = 'm-0 mb-3 text-[13px] font-semibold text-[#888] uppercase tracking-[0.05em]'
  const inputClass = 'px-3 py-2 rounded-md border border-[#333] bg-[#121218] text-[#e0e0e0] text-[13px] outline-none transition-colors duration-150 focus:border-[#8142c7]'

  return (
    <div className="relative min-h-full">
      {/* Setup overlay */}
      {setupProgress && (
        <div className="absolute inset-0 bg-[rgba(18,18,24,0.92)] z-10 flex items-center justify-center backdrop-blur-[4px]">
          <div className="flex flex-col items-center gap-4 px-10 py-8 bg-[#1a1a2e] border border-[#2a2a3a] rounded-xl text-[#e0e0e0] max-w-[340px] w-full">
            ...
            {setupProgress.detail && <div className="text-[11px] text-[#666] text-center">{setupProgress.detail}</div>}
            <div className="w-full h-[3px] bg-[#2a2a3a] rounded-full overflow-hidden">
              <div
                className="h-full bg-[#8142c7] rounded-full transition-[width] duration-400"
      ...
      <section className="mb-8">
        ...
        <p className="m-0 mb-4 text-[13px] text-[#666] leading-[1.5]">
        ...
        <form onSubmit={handleAddDoc} className="bg-[#1a1a2a] border border-[#2a2a3a] rounded-lg p-4">
          ...
          <div className="flex gap-2 mb-2.5">
            <label className="text-[12px] text-[#888] whitespace-nowrap self-center">URL prefix:</label>
            <input
              type="text"
          ...
          <div className="flex gap-2 items-center">
            <label className="text-[12px] text-[#888] flex items-center gap-1.5">
              Crawl depth:
              <select
                value={docCrawlDepth}
                onChange={(e) => setDocCrawlDepth(parseInt(e.target.value))}
                className="px-2 py-1 rounded border border-[#333] bg-[#121218] text-[#ccc] text-[12px]"
              >
            ...
            <button
              ...
              className="px-4 py-2 rounded-md border-none bg-[#8142c7] text-white text-[13px] font-medium cursor-pointer flex items-center gap-1.5 transition-colors duration-150 hover:bg-[#9555d9] disabled:opacity-50 disabled:cursor-not-allowed"
            >
      ...
      {(docError || docsAPI?.error) && (
        <div className="my-3 px-3 py-2 rounded-md bg-[rgba(255,80,80,0.1)] border border-[rgba(255,80,80,0.3)] text-[#ff5050] text-[13px]">
          {docError || docsAPI?.error}
        </div>
      )}
      ...
      <section className="mb-8">
        <div className="flex items-center justify-between mb-3">
          <h3 className={`${sectionTitleClass} !mb-0`}>
            ...
            {docsAPI && docsAPI.sources.length > 0 && (
              <span className="font-normal text-[#666] normal-case tracking-normal"> ({docsAPI.sources.length})</span>
            )}
          </h3>
          <button
            className="p-[5px] rounded border border-[#333] bg-transparent text-[#888] cursor-pointer flex items-center justify-center transition-colors duration-150 hover:text-[#ccc] hover:bg-[#2a2a3a] hover:border-[#3a3a5a] disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        {docsAPI?.loading ? (
          <div className="flex items-center gap-2 text-[#666] text-[13px] py-4">
          ...
          <div className="flex flex-col gap-2">
            {docsAPI.sources.map((source) => (
              <div key={source.id} className="px-4 py-3 rounded-lg border border-[#2a2a3a] bg-[#1a1a2a] transition-colors duration-150 hover:border-[#3a3a5a]">
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-2">
                    {source.pageCount > 0 ? (
                      <button
                        className="bg-transparent border-none p-0 m-0 cursor-pointer text-[#888] flex items-center shrink-0 hover:text-[#ccc]"
                  ...
                  <div className="flex gap-1">
                    {source.status === 'crawling' ? (
                      <button
                        ...
                        title="Stop crawl"
                        className="p-1 rounded border-none bg-transparent text-[#666] cursor-pointer transition-colors duration-150 hover:text-[#ff5050] hover:bg-[#2a2a3a]"
                      >
                        <Square size={14} />
                      </button>
                    ) : (
                      <>
                        <button
                          ...
                          title="Re-crawl"
                          className="p-1 rounded border-none bg-transparent text-[#666] cursor-pointer transition-colors duration-150 hover:text-[#ccc] hover:bg-[#2a2a3a]"
                        >
                          <RefreshCw size={14} />
                        </button>
                        <button
                ...
                <div className="flex items-center gap-3 text-[12px] text-[#666]">
                  <a href={source.url} target="_blank" rel="noopener noreferrer" className="text-inherit no-underline flex items-center gap-[3px] transition-colors duration-150 hover:text-[#999]">
                ...
                {source.status === 'crawling' && crawlProgress[source.id]?.currentUrl && (
                  <div className="mt-1 text-[11px] text-[#888] overflow-hidden text-ellipsis whitespace-nowrap">
                    {crawlProgress[source.id].currentUrl}
                  </div>
                )}
                {source.status === 'error' && source.errorMessage && (
                  <div className="mt-1.5 text-[12px] text-[#ff5050]">
                    Error: {source.errorMessage}
                  </div>
                )}
                {expandedSource === source.id && (
                  <div className="max-h-[200px] overflow-y-auto border-t border-[#2a2a3a] mt-2 pt-1">
                    {loadingPages === source.id ? (
                      <div className="flex items-center gap-1.5 px-2 py-1.5 text-[12px] text-[#666]">
                        <Loader2 size={12} className={spinClass} /> Loading pages...
                      </div>
                    ) : sourcePages[source.id]?.length ? (
                      sourcePages[source.id].map((page) => (
                        <div key={page.id} className="flex items-center gap-2 px-2 py-[3px] text-[12px] text-[#aaa] overflow-hidden hover:bg-[rgba(255,255,255,0.04)]">
                          <span className="whitespace-nowrap overflow-hidden text-ellipsis min-w-0 flex-1">{page.title || page.url}</span>
                          <a href={page.url} target="_blank" rel="noopener noreferrer" className="whitespace-nowrap overflow-hidden text-ellipsis text-[#555] text-[11px] shrink-0 max-w-[40%] no-underline hover:text-[#888]">
                            {page.url}
                          </a>
                        </div>
                      ))
                    ) : (
                      <div className="flex items-center gap-1.5 px-2 py-1.5 text-[12px] text-[#666]">No pages found</div>
                    )}
                  </div>
                )}
              </div>
      ...
      {/* Crawl Engine */}
      <section className="mb-8 border-t border-[#2a2a3a] mt-2 pt-4">
        <h3 className={sectionTitleClass}>Crawl Engine</h3>
        <div className="flex items-center justify-between gap-4">
          <span className="text-[13px] text-[#888]">Python runtime used for crawling documentation sites.</span>
          <button
            className="flex items-center gap-1.5 px-3.5 py-1.5 bg-transparent border border-[#333] rounded-md text-[#ccc] text-[13px] cursor-pointer whitespace-nowrap transition-colors duration-150 hover:enabled:bg-[#2a2a3a] hover:enabled:border-[#444] disabled:opacity-50 disabled:cursor-not-allowed"
