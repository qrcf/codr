...
const variantStyles = {
  permission: { border: 'border-2 border-accent', bg: 'bg-[#1e1e2a]', toggleColor: 'text-[#a0b0ff]' },
  plan: { border: 'border-2 border-accent', bg: 'bg-[#1e1e2e]', toggleColor: 'text-[#a0b0ff]' },
...
export function CollapsibleDialog({ title, icon, variant, children, defaultCollapsed = false }: CollapsibleDialogProps) {
  ...
  return (
    <div className={`rounded-lg overflow-hidden ${vs.border} ${vs.bg}`}>
      ...
      <div className={hasScrollBody ? 'flex flex-col max-h-[calc(70vh-50px)] overflow-hidden' : ''}>
        {!collapsed && children}
      </div>
    </div>
  )
