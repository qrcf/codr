...
export function ChatHeader({ sidebarOpen, onToggleSidebar, projectTitle, activeSession, isDraft, isPendingNewChat, allProjects, onChangeProject, approvedPlan, onShowPlan, onOpenManageProject, onRegenTitle, currentProvider, onProviderChange, canChangeProvider }: ChatHeaderProps) {
  ...
  // Close provider dropdown on outside click / Escape
  useEffect(() => {
    ...
    const handleKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') setProviderDropdownOpen(false)
    }
  ...
  return (
    <header className="flex items-center gap-3 px-4 py-3 border-b border-border shrink-0 max-[768px]:px-3 max-[768px]:py-2.5 max-[768px]:gap-2">
      ...
      {(currentProvider || approvedPlan) && (
        <div className="ml-auto flex items-center gap-2 shrink-0">
          {currentProvider && (() => {
            const theme = PROVIDER_THEME[currentProvider]
            if (!theme) return null
            return (
              <div className="relative" ref={providerDropdownRef}>
                <button
                  className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-[0.78em] font-medium border transition-colors duration-150 ${
                    canChangeProvider
                      ? 'cursor-pointer border-border hover:border-border-subtle hover:bg-bg-card text-text-muted hover:text-[#ccc]'
                      : 'cursor-default border-transparent text-text-dim'
                  }`}
                  onClick={() => { if (canChangeProvider) setProviderDropdownOpen(prev => !prev) }}
                  title={canChangeProvider ? 'Change provider' : theme.label}
                >
                  <ProviderLogo providerId={currentProvider} size={13} className="opacity-80" />
                  <span>{theme.label}</span>
                  {canChangeProvider && <ChevronDown size={11} className={`transition-transform duration-150 ${providerDropdownOpen ? 'rotate-180' : ''}`} />}
                </button>
                {providerDropdownOpen && canChangeProvider && (
                  <div className="absolute top-full right-0 mt-1 bg-bg-card border border-border rounded-md py-1 z-100 shadow-[0_4px_12px_rgba(0,0,0,0.4)] min-w-44">
                    {PROVIDER_IDS.map(id => {
                      ...
                      return (
                        <button
                          key={id}
                          className={`w-full flex items-center gap-2 px-3 py-2 text-[0.83em] bg-transparent border-none text-left cursor-pointer transition-colors duration-100 ${
                            isActive ? 'text-[#ccc] bg-[#2a2a3e]' : 'text-[#aaa] hover:bg-[#2a2a3e] hover:text-white'
                          }`}
                          onClick={() => {
                            onProviderChange?.(id)
                            setProviderDropdownOpen(false)
          ...
          {approvedPlan && (
            <button
              className="flex items-center gap-1.5 bg-transparent border border-[#3a4a4a] text-[#78c0a8] rounded-md px-3 py-1.5 text-[0.8em] font-medium cursor-pointer hover:bg-[#242a2a] transition-colors"
              onClick={onShowPlan}
            >
